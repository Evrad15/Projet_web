...
<div class="container py-4">
    ...
    <div class="card mb-4 border-0 shadow-sm">
        <div class="card-body">
            <form method="GET" action="{{ route('dashboard.movements') }}" class="row g-3">
                <div class="col-md-4">
                    <div class="input-group">
                        <span class="input-group-text bg-light border-0"><i class="bi bi-search text-muted"></i></span>
                        <input type="text" name="search" class="form-control bg-light border-0" placeholder="Rechercher un produit..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="col-md-3">
                    <select name="type" class="form-select bg-light border-0">
                        <option value="">Tous les types</option>
                        <option value="supply" {{ request('type') == 'supply' ? 'selected' : '' }}>Approvisionnement</option>
                        <option value="sale" {{ request('type') == 'sale' ? 'selected' : '' }}>Vente</option>
                        <option value="correction" {{ request('type') == 'correction' ? 'selected' : '' }}>Correction / Inventaire</option>
                        <option value="return" {{ request('type') == 'return' ? 'selected' : '' }}>Retour Client</option>
                        <option value="initial" {{ request('type') == 'initial' ? 'selected' : '' }}>Stock Initial</option>
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <a href="{{ route('dashboard.movements') }}" class="btn btn-outline-secondary w-100">Réinitialiser</a>
                </div>
                @endif
            </form>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4 text-muted small">{{ $m->created_at->format('d/m/Y H:i') }}</td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="badge-type {{ $style[0] }}">{{ $style[1] }}</span>
                            </td>
                            <td>
                                <span class="fw-bold {{ $m->quantity > 0 ? 'text-success' : 'text-danger' }}">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <small>{{ $m->user->name ?? 'Système' }}</small>
                                </div>
                            </td>
